...
#include <stdlib.h>
#define size 10
struct priorityqueue1
{
    int item[size];
    int rear, front;
    int priority[size];
}q;
void enqueue(struct priorityqueue1 *q, int x);
void dequeue(struct priorityqueue1 *q);
void display(struct priorityqueue1 *q);
int main()
{
    q.rear = size-1;
    q.front = size-1;
...
void display(struct priorityqueue1 *q)
{
    ...
    {
        printf("%d, priority = %d\t", q->item[i], q->priority[i]);
    }
}